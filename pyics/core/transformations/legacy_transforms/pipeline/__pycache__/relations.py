#!/usr/bin/env python3
"""
pyics/core/transforms/pipeline/relations.py
Pyics Core Domain Relations: transforms/pipeline
...
PROBLEM SOLVED: Centralized relationship mapping for transforms/pipeline domain
DEPENDENCIES: transforms/pipeline.data_types, typing, dataclasses
...
in the transforms/pipeline domain, following DOP principles with immutable
...
class RelationType(Enum):
    """Types of relationships in transforms/pipeline domain"""
    ONE_TO_ONE = auto()
    ONE_TO_MANY = auto()
    MANY_TO_MANY = auto()
    HIERARCHICAL = auto()
...
class Relation:
    """
    Immutable relation between transforms/pipeline entities
...
class RelationGraph:
    """
    Immutable graph of relations in transforms/pipeline domain
    
    Container for complete relationship structure
    """
    relations: Tuple[Relation, ...] = field(default_factory=tuple)
    entity_index: Dict[str, Transforms/PipelineEntity] = field(default_factory=dict)
    ...
    def get_related_entities(self, entity_id: str) -> List[Transforms/PipelineEntity]:
...
class RelationMapping:
    """
    Static mapping configuration for transforms/pipeline relations
...
# Predefined relation mappings for transforms/pipeline domain
DEFAULT_RELATION_MAPPINGS: List[RelationMapping] = [
    RelationMapping(
        mapping_name="hierarchical_parent_child",
        source_type="Transforms/PipelineEntity",
        target_type="Transforms/PipelineEntity",
...
# [EOF] - End of transforms/pipeline relations.py module
