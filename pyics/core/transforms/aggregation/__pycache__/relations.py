#!/usr/bin/env python3
"""
pyics/core/transforms/aggregation/relations.py
Pyics Core Domain Relations: transforms/aggregation
...
PROBLEM SOLVED: Centralized relationship mapping for transforms/aggregation domain
DEPENDENCIES: transforms/aggregation.data_types, typing, dataclasses
...
in the transforms/aggregation domain, following DOP principles with immutable
...
class RelationType(Enum):
    """Types of relationships in transforms/aggregation domain"""
    ONE_TO_ONE = auto()
    ONE_TO_MANY = auto()
    MANY_TO_MANY = auto()
    HIERARCHICAL = auto()
...
class Relation:
    """
    Immutable relation between transforms/aggregation entities
...
class RelationGraph:
    """
    Immutable graph of relations in transforms/aggregation domain
    
    Container for complete relationship structure
    """
    relations: Tuple[Relation, ...] = field(default_factory=tuple)
    entity_index: Dict[str, Transforms/AggregationEntity] = field(default_factory=dict)
    ...
    def get_related_entities(self, entity_id: str) -> List[Transforms/AggregationEntity]:
...
class RelationMapping:
    """
    Static mapping configuration for transforms/aggregation relations
...
# Predefined relation mappings for transforms/aggregation domain
DEFAULT_RELATION_MAPPINGS: List[RelationMapping] = [
    RelationMapping(
        mapping_name="hierarchical_parent_child",
        source_type="Transforms/AggregationEntity",
        target_type="Transforms/AggregationEntity",
...
# [EOF] - End of transforms/aggregation relations.py module
