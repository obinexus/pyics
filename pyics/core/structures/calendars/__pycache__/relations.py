#!/usr/bin/env python3
"""
pyics/core/structures/calendars/relations.py
Pyics Core Domain Relations: structures/calendars
...
PROBLEM SOLVED: Centralized relationship mapping for structures/calendars domain
DEPENDENCIES: structures/calendars.data_types, typing, dataclasses
...
in the structures/calendars domain, following DOP principles with immutable
...
class RelationType(Enum):
    """Types of relationships in structures/calendars domain"""
    ONE_TO_ONE = auto()
    ONE_TO_MANY = auto()
    MANY_TO_MANY = auto()
    HIERARCHICAL = auto()
...
class Relation:
    """
    Immutable relation between structures/calendars entities
...
class RelationGraph:
    """
    Immutable graph of relations in structures/calendars domain
    
    Container for complete relationship structure
    """
    relations: Tuple[Relation, ...] = field(default_factory=tuple)
    entity_index: Dict[str, Structures/CalendarsEntity] = field(default_factory=dict)
    ...
    def get_related_entities(self, entity_id: str) -> List[Structures/CalendarsEntity]:
...
class RelationMapping:
    """
    Static mapping configuration for structures/calendars relations
...
# Predefined relation mappings for structures/calendars domain
DEFAULT_RELATION_MAPPINGS: List[RelationMapping] = [
    RelationMapping(
        mapping_name="hierarchical_parent_child",
        source_type="Structures/CalendarsEntity",
        target_type="Structures/CalendarsEntity",
...
# [EOF] - End of structures/calendars relations.py module
