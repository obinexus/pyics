#!/usr/bin/env python3
"""
pyics/core/registry/versioned/relations.py
Pyics Core Domain Relations: registry/versioned
...
PROBLEM SOLVED: Centralized relationship mapping for registry/versioned domain
DEPENDENCIES: registry/versioned.data_types, typing, dataclasses
...
in the registry/versioned domain, following DOP principles with immutable
...
class RelationType(Enum):
    """Types of relationships in registry/versioned domain"""
    ONE_TO_ONE = auto()
    ONE_TO_MANY = auto()
    MANY_TO_MANY = auto()
    HIERARCHICAL = auto()
...
class Relation:
    """
    Immutable relation between registry/versioned entities
...
class RelationGraph:
    """
    Immutable graph of relations in registry/versioned domain
    
    Container for complete relationship structure
    """
    relations: Tuple[Relation, ...] = field(default_factory=tuple)
    entity_index: Dict[str, Registry/VersionedEntity] = field(default_factory=dict)
    ...
    def get_related_entities(self, entity_id: str) -> List[Registry/VersionedEntity]:
...
class RelationMapping:
    """
    Static mapping configuration for registry/versioned relations
...
# Predefined relation mappings for registry/versioned domain
DEFAULT_RELATION_MAPPINGS: List[RelationMapping] = [
    RelationMapping(
        mapping_name="hierarchical_parent_child",
        source_type="Registry/VersionedEntity",
        target_type="Registry/VersionedEntity",
...
# [EOF] - End of registry/versioned relations.py module
