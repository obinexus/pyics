#!/usr/bin/env python3
"""
pyics/core/registry/migration/relations.py
Pyics Core Domain Relations: registry/migration
...
PROBLEM SOLVED: Centralized relationship mapping for registry/migration domain
DEPENDENCIES: registry/migration.data_types, typing, dataclasses
...
in the registry/migration domain, following DOP principles with immutable
...
class RelationType(Enum):
    """Types of relationships in registry/migration domain"""
    ONE_TO_ONE = auto()
    ONE_TO_MANY = auto()
    MANY_TO_MANY = auto()
    HIERARCHICAL = auto()
...
class Relation:
    """
    Immutable relation between registry/migration entities
...
class RelationGraph:
    """
    Immutable graph of relations in registry/migration domain
    
    Container for complete relationship structure
    """
    relations: Tuple[Relation, ...] = field(default_factory=tuple)
    entity_index: Dict[str, Registry/MigrationEntity] = field(default_factory=dict)
    ...
    def get_related_entities(self, entity_id: str) -> List[Registry/MigrationEntity]:
...
class RelationMapping:
    """
    Static mapping configuration for registry/migration relations
...
# Predefined relation mappings for registry/migration domain
DEFAULT_RELATION_MAPPINGS: List[RelationMapping] = [
    RelationMapping(
        mapping_name="hierarchical_parent_child",
        source_type="Registry/MigrationEntity",
        target_type="Registry/MigrationEntity",
...
# [EOF] - End of registry/migration relations.py module
