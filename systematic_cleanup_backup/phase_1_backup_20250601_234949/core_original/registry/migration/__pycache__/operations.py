#!/usr/bin/env python3
"""
pyics/core/registry/migration/operations.py
Pyics Core Domain Operations: registry/migration
...
PROBLEM SOLVED: Centralized operation definitions for registry/migration domain
DEPENDENCIES: registry/migration.data_types, registry/migration.relations, typing
THREAD SAFETY: Yes - pure functions with immutable data
DETERMINISTIC: Yes - deterministic operations on immutable data

This module provides atomic and composed operations for the registry/migration
...
logger = logging.getLogger(f"pyics.core.registry/migration.operations")
...
def create_entity(
    ...
    **metadata
    ...
    """
    Create a new registry/migration entity
    
    Pure function for entity creation
    """
    return Registry/MigrationEntity(
        id=entity_id,
        name=name,
        status=status,
        priority=priority,
...
def update_entity_status(
    entity: Registry/MigrationEntity,
    new_status: Registry/MigrationStatus
) -> Registry/MigrationEntity:
    """
...
def update_entity_priority(
    entity: Registry/MigrationEntity,
    new_priority: Registry/MigrationPriority
) -> Registry/MigrationEntity:
    """
...
def filter_entities_by_status(
    entities: Registry/MigrationCollection,
    status: Registry/MigrationStatus
) -> Registry/MigrationCollection:
    """
...
def filter_entities_by_priority(
    entities: Registry/MigrationCollection,
    min_priority: Registry/MigrationPriority
) -> Registry/MigrationCollection:
    """
...
def group_entities_by_status(
    entities: Registry/MigrationCollection
) -> Dict[Registry/MigrationStatus, Registry/MigrationCollection]:
    """
    Group entities by status
    
    Pure grouping function
    """
    groups: Dict[Registry/MigrationStatus, Registry/MigrationCollection] = {}
...
def merge_entity_indices(
    *indices: Registry/MigrationIndex
) -> Registry/MigrationIndex:
    """
...
def filter_index_by_predicate(
    index: Registry/MigrationIndex,
    predicate: Callable[[Registry/MigrationEntity], bool]
) -> Registry/MigrationIndex:
    """
...
def process_entity_collection(
    entities: Registry/MigrationCollection,
    operations: List[Callable[[Registry/MigrationEntity], Registry/MigrationEntity]]
) -> Registry/MigrationCollection:
    """
    ...
    """
    def apply_operations(entity: Registry/MigrationEntity) -> Registry/MigrationEntity:
...
def transform_collection_with_config(
    entities: Registry/MigrationCollection,
    config: Registry/MigrationConfig
) -> Registry/MigrationResult:
    """
    ...
    try:
        ...
        if not config.enabled:
            return Registry/MigrationResult(
                success=True,
        ...
        for entity in filtered_entities:
            ...
            if config.options.get("auto_priority_boost", False):
                entity = update_entity_priority(entity, Registry/MigrationPriority.HIGH)
...
def validate_entity_collection(
    entities: Registry/MigrationCollection,
    validation_rules: List[Callable[[Registry/MigrationEntity], bool]]
) -> Registry/MigrationResult:
    """
...
STANDARD_ENTITY_OPERATIONS = [
    partial_operation(update_entity_status, new_status=Registry/MigrationStatus.PROCESSING),
]

PRIORITY_BOOST_OPERATIONS = [
    partial_operation(update_entity_priority, new_priority=Registry/MigrationPriority.HIGH),
...
# [EOF] - End of registry/migration operations.py module
